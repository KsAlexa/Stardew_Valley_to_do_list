...
def test_find_initial_day_with_get_active(repo_with_initial_day: DayRepository):
    active_day = repo_with_initial_day.get_active()
    assert active_day is not None, 'Initial day was not created'
    assert active_day.id == 1, f'Expected id=1, got {active_day.id} instead'
    assert active_day.active is True, 'Initial day isn\'t active'

def test_deactivate_initial_day(repo_with_initial_day: DayRepository):
    initial_active_day = repo_with_initial_day.get_active()
    assert initial_active_day is not None and initial_active_day.id == 1, 'Incorrect DB initial state'
...
# def test_set_activity_on_non_existing_day(repo_with_initial_day: DayRepository):
...
def test_get_by_id_with_no_such_day(repo_with_initial_day: DayRepository):
    initial_active_day = repo_with_initial_day.get_active()
    assert initial_active_day is not None and initial_active_day.id == 1, 'Incorrect DB initial state'
    assert repo_with_initial_day.get_by_id(2) is None, 'Should return None if no day with such id'

def test_insert_existing_day_does_nothing(repo_with_initial_day: DayRepository):
    ...
    try:
        ...
        pytest.fail(f'insert() got Exception "{e}", but it shouldn\'t happen')
    ...
    assert initial_day_after_conflict.id == initial_day.id, 'Day after conflict changed his id'
    assert initial_day_after_conflict.active is True, 'Day after conflict changed his activity'
...
def test_get_day_by_attributes_with_no_such_day(repo_with_multiple_days_data: DayRepository):
    search_year, search_season, search_day = 1, 'summer', 11
    day_from_db = repo_with_multiple_days_data.get_by_attributes(search_year, search_season, search_day)
    assert day_from_db is None, 'Should return None if no day with such attributes'
